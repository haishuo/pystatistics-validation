...
**8 of 9 done. HOME STRETCH — `ordinal+multinomial` (#8) CHIP SPAWNED (task_5acf1f5e); the
last module is anova/descriptive/hypothesis (#9).**
...
| 8 | ordinal (`polr`) + multinomial (`multinom`) | 🔄 **CHIP SPAWNED** (task_5acf1f5e) — FULL run + red-team in ONE chip | two modules: `ordinal.polr` vs `MASS::polr`, `multinomial.multinom` vs `nnet::multinom` (decay=0). Two-tier MLE contract (β/log-lik optimizer-tier; fitted probs tight). R10 separation central. **⚠️ CF-1 CHECK MANDATORY** — both form the IRLS Hessian `X'WX`; chip first determines if either exposes a GPU path, and if so proves no fp32 silent-wrong on MPS+CUDA (gam materialised it). Target PyPI 4.6.8. |
