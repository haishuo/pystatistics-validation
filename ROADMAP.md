...
**7 of 9 done. HOME STRETCH — `montecarlo` (#7) CHIP SPAWNED (task_461d80e8); 2 more after:
...
| 7 | montecarlo (bootstrap/permutation) | 🔄 **CHIP SPAWNED** (task_461d80e8) — FULL run + red-team in ONE chip | `boot`/`boot_ci`/`permutation_test` vs `boot::`. STOCHASTIC-method contract: same-resamples tight tier (feed identical indices → exact BCa z0/a + all CI types) + statistical-equivalence tier (large-B convergence + coverage within MC error) + R6 seed reproducibility. Real GPU-win candidate (embarrassingly parallel; CF-1 N/A — no Gram; R11/R12/R13 + fp32-accumulation-drift + GPU-RNG checks if GPU). Target PyPI 4.6.6. |
